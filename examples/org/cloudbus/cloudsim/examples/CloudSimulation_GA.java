...
public class CloudSimulation {
    ...
    public static void main(String[] args) {
        ...
        Log.printLine("Starting Cloud Simulation with Genetic Algorithm...");

        try {
            // Run simulation for SDSC dataset
            runSDSCDataset();
    ...
    /**
     * Runs simulation for SDSC dataset with 10 iterations
     */
    private static void runSDSCDataset() throws Exception {
        // Create CSV file for results
        BufferedWriter resultsWriter = new BufferedWriter(new FileWriter("results_SDSC_GA.csv"));
        
        // Write headers to CSV file
        writeCSVHeader(resultsWriter);
        
        System.out.println("==================================================");
        System.out.println("Starting simulation with SDSC dataset");
        System.out.println("1 dataset × 10 iterations = 10 total iterations");
        System.out.println("==================================================");
        
        // Run SDSC dataset
        System.out.println("\nProcessing SDSC dataset...");
        ...
        int datasetSize = 7395; // SDSC dataset has 7395 tasks
        
        for (int i = 0; i < iterationCount; i++) {
            System.out.println("  └─ Iteration " + (i+1) + "/" + iterationCount + " (" + (i+1) + "/10 total)");
            Log.printLine("SDSC Iteration " + (i+1) + " of " + iterationCount);
            SimulationResult result = runSimulation(sdcsFile, datasetSize);
            writeResultToCSV(resultsWriter, result, "SDSC", i+1);
        ...
        System.out.println("\n==================================================");
        System.out.println("All 10 iterations completed successfully!");
        System.out.println("Results saved to: results_SDSC_GA.csv");
        System.out.println("==================================================");
    }
    ...
    private static SimulationResult runSimulation(String datasetPath, int datasetSize) throws Exception {
        ...
        System.out.println("      └─ Running Genetic Algorithm for task scheduling...");
        for (int cloudletIterator = 0; cloudletIterator <= cloudletLoopingNumber; cloudletIterator++) {
            ...
            for (int dataCenterIterator = 1; dataCenterIterator <= 6; dataCenterIterator++) {
                ...
                int populationSize = 10; // Population size (number of chromosomes)
                double crossoverProbability = 0.8; // Probability of crossover
                double mutationProbability = 0.1; // Probability of mutation

                // Initialize Genetic Algorithm
                GeneticAlgorithm geneticAlgorithm = new GeneticAlgorithm(maxIterations, populationSize, 
                        crossoverProbability, mutationProbability, cloudletList, vmlist, cloudletNumber);

                // Initialize population
                System.out.println("            └─ Initializing population");
                PopulationGA population = geneticAlgorithm.initPopulation(cloudletNumber, dataCenterIterator);

                // Run the Genetic Algorithm
                System.out.println("            └─ Running Genetic Algorithm");
                geneticAlgorithm.runGA(population, dataCenterIterator);

                // Get the best solution
                int[] bestSolution = geneticAlgorithm.getBestVmAllocationForDatacenter(dataCenterIterator);
